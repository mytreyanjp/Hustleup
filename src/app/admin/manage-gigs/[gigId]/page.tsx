...
import { Loader2, ArrowLeft, UserCircle, Briefcase, CalendarDays, FileText, MessageSquare, Users, Layers, Star, Trash2, ShieldAlert, Edit, CreditCard, Send, IndianRupee } from 'lucide-react'; // Replaced DollarSign with IndianRupee
  ...
  const handleReleasePayment = async () => {
    ...
    try {
        ...
        await createAdminActionNotification(
            ...
            `Payment of ₹${gig.budget.toFixed(2)} for the gig "${gig.title}" has been released to your account by an administrator.`,
            'payment_released',
            gig.id,
            gig.title,
            adminUser.uid,
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground flex items-center"><IndianRupee className="mr-1.5 h-4 w-4" /> ₹{gig.budget.toFixed(2)}</p>
            </div>
            <div>
            ...
            <CardFooter className="border-t pt-4">
                <div className="flex flex-col sm:flex-row items-center justify-between w-full gap-2">
                    <p className="text-sm font-medium text-primary">
                        Client has paid. Payment of ₹{gig.budget.toFixed(2)} is awaiting release to {selectedStudentProfile?.username || 'the student'}.
                    </p>
                    <Button 
                ...
                <TableBody>
                  {gigTransactions.map(tx => (
                    <TableRow key={tx.id}>
                      ...
                      <TableCell className="text-xs sm:text-sm">₹{tx.amount.toFixed(2)}</TableCell>
                      <TableCell>
