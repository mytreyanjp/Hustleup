...
import { doc, getDoc, updateDoc, Timestamp, setDoc, collection, addDoc, serverTimestamp, writeBatch, query, where, getDocs, arrayUnion, increment, deleteDoc as firestoreDeleteDoc, arrayRemove } from 'firebase/firestore'; // Renamed deleteDoc to firestoreDeleteDoc
import { db, storage } from '@/config/firebase';
import { ref as storageRefFn, deleteObject } from 'firebase/storage';
...
export default function AdminGigDetailPage() {
  ...
  const [attachmentToDelete, setAttachmentToDelete] = useState<{ gigId: string; reportNumber: number; attachment: Attachment } | null>(null);
  ...
  const handleDeleteGig = async () => {
    ...
    try {
      await firestoreDeleteDoc(doc(db, 'gigs', gigId));
  ...
  const confirmDeleteAttachment = async () => {
    if (!attachmentToDelete || !db || !adminUser || !storage) {
      toast({ title: "Error", description: "Cannot delete attachment. Missing critical data or services.", variant: "destructive" });
      return;
    }
    const { gigId: currentGigId, reportNumber, attachment } = attachmentToDelete;
    ...
    try {
      ...
      try {
        ...
        // Firebase Storage URLs are usually /v0/b/{bucket}/o/{path_to_file}?alt=media&token=...
        ...
        console.log("Attachment deleted from Firebase Storage:", attachment.name);
      } catch (storageError: any) {
        if (storageError.code === 'storage/object-not-found') {
            toast({ title: "Attachment Not Found in Storage", description: "The file may have already been deleted or was not uploaded correctly. Removing reference.", variant: "default" });
        } else if (storageError.code === 'storage/unauthorized') {
            toast({ title: "Storage Permission Denied", description: "You do not have permission to delete this file from storage. Please check Firebase Storage rules.", variant: "destructive", duration: 10000 });
            setIsDeletingAttachment(false);
            setAttachmentToDelete(null);
            return;
        } else {
            console.error("Error deleting attachment from Firebase Storage:", storageError);
            toast({ title: "Storage Deletion Error", description: `Could not delete file from storage: ${storageError.message}. Proceeding to remove reference.`, variant: "destructive" });
        }
      }

      // 2. Update Firestore document
      const gigDocRef = doc(db, 'gigs', currentGigId);
      const gigSnap = await getDoc(gigDocRef);
      if (!gigSnap.exists()) throw new Error("Gig document not found for updating attachments.");
      const currentGigData = gigSnap.data() as Gig;
      
      const updatedProgressReports = (currentGigData.progressReports || []).map(pr => {
        if (pr.reportNumber === reportNumber && pr.studentSubmission && pr.studentSubmission.attachments) {
          return {
            ...pr,
            studentSubmission: {
              ...pr.studentSubmission,
              attachments: pr.studentSubmission.attachments.filter(att => att.url !== attachment.url)
            }
          };
        }
        return pr;
      ...
      // Refresh local gig state
      setGig(prev => prev ? { ...prev, progressReports: updatedProgressReports } : null);
      
      toast({ title: "Attachment Removed", description: `Attachment "${attachment.name}" has been removed from Report #${reportNumber}.` });

      // 3. Notify relevant users
      if (gig) {
          await createAdminActionNotification(
            gig.clientId,
            `An attachment ("${attachment.name}") from Report #${reportNumber} for your gig "${gig.title}" was removed by an administrator.`,
            'report_attachment_deleted',
            gig.id,
            gig.title,
            adminUser.uid,
            adminProfile?.username
          ...
          if (gig.selectedStudentId) {
            await createAdminActionNotification(
              gig.selectedStudentId,
              `Your attachment ("${attachment.name}") for Report #${reportNumber} on the gig "${gig.title}" was removed by an administrator.`,
              'report_attachment_deleted',
              gig.id,
              gig.title,
              adminUser.uid,
      ...
      console.error("Error deleting attachment reference from Firestore:", error);
      toast({ title: "Firestore Update Error", description: `Could not remove attachment reference: ${error.message}`, variant: "destructive" });
    } finally {
      setIsDeletingAttachment(false);
      setAttachmentToDelete(null);
    }
  ...
  const getStatusBadgeVariant = (status: Gig['status'] | ApplicantInfo['status'] | ProgressReport['clientStatus'] | GigTransaction['status']): "default" | "secondary" | "destructive" | "outline" => {
       switch (status) {
           case 'open': case 'accepted': case 'approved': case 'succeeded': case 'payout_to_student_succeeded': return 'default'; 
           case 'in-progress': case 'pending': case 'pending_review': case 'pending_release_to_student': return 'secondary'; 
           case 'awaiting_payout': return 'outline'; 
                  ...
                  <TableRow key={applicant.studentId}>
                    ...
                    <TableCell className="text-right">
                       <div className="flex flex-col sm:flex-row gap-1.5 justify-end items-center">
                         ...
                         <Button variant="destructive" size="sm" className="text-xs" onClick={() => {setApplicantToDelete(applicant); setShowDeleteApplicantDialog(true);}} disabled={isDeletingApplicant && applicantToDelete?.studentId === applicant.studentId}>
              ...
              <Card key={report.reportNumber} className="bg-muted/30 p-3">
                ...
                {report.studentSubmission ? (
                  <div className="text-xs space-y-1">
                            ...
                            {report.studentSubmission.attachments.map((att, idx) => (
                                <div key={idx} className="flex items-center gap-2">
                                    ...
                                    <Button 
                                        ...
                                        onClick={() => setAttachmentToDelete({ gigId: gig.id, reportNumber: report.reportNumber, attachment: att})}
                                        disabled={isDeletingAttachment}
                                        title={`Delete attachment: ${att.name}`}
                                    >
                    ...
                    ) : report.studentSubmission.fileUrl ? ( // Fallback for old single file
                        <div className="flex items-center gap-2">
                           <Button variant="link" size="xs" asChild className="p-0 h-auto text-xs">
                              <a href={report.studentSubmission.fileUrl} target="_blank" rel="noopener noreferrer">
                                 <FileText className="mr-1 h-3 w-3" /> {report.studentSubmission.fileName || 'View Attachment'}
                              </a>
                           </Button>
                           <Button 
                                variant="ghost" 
                                size="icon" 
                                className="h-5 w-5 text-destructive hover:bg-destructive/10"
                                onClick={() => setAttachmentToDelete({ gigId: gig.id, reportNumber: report.reportNumber, attachment: { name: report.studentSubmission.fileName || 'attachment', url: report.studentSubmission.fileUrl! }})}
                                disabled={isDeletingAttachment}
                                title={`Delete attachment: ${report.studentSubmission.fileName || 'attachment'}`}
                            >
                                <Trash2 className="h-3 w-3" />
                           </Button>
                        </div>
        ...
        <AlertDialog open={!!attachmentToDelete} onOpenChange={(isOpen) => !isOpen && setAttachmentToDelete(null)}>
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Are you sure you want to delete the attachment "{attachmentToDelete?.attachment.name}" from Report #{attachmentToDelete?.reportNumber}? This action cannot be undone.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel disabled={isDeletingAttachment}>Cancel</AlertDialogCancel>
                    <AlertDialogAction onClick={confirmDeleteAttachment} disabled={isDeletingAttachment} className="bg-destructive hover:bg-destructive/90">
                        {isDeletingAttachment ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
